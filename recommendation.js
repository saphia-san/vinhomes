...
function renderAllMatchedCards() {
    ...
    container.innerHTML = allMatchingResults.map(item => {
        ...
        const vosBadge = isVOS ? `<span class="badge bg-warning text-dark fw-bold shadow-sm px-2 py-0.5" style="font-size: 0.72rem;"><i class="bi bi-house-check-fill me-1"></i>VOS: -5% HĐMB + 5% tiền mặt</span>` : '';
        const goldBadge = `<span class="badge bg-danger bg-gradient text-white fw-bold shadow-sm px-2 py-0.5" style="font-size: 0.72rem;"><i class="bi bi-gift-fill me-1"></i>${goldText}</span>`;
        const gianXayBadge = u.type === 'gianXay' ? `<span class="badge bg-info text-dark fw-bold shadow-sm px-2 py-0.5" style="font-size: 0.72rem;"><i class="bi bi-hourglass-split me-1"></i>Giãn Xây Q4/2028</span>` : '';
                ...
                <div class="card-recommendation-item rounded-4 h-100 d-flex flex-column justify-content-between shadow position-relative"
                    ...
                    <div>
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-2 flex-wrap gap-1">
                            <div class="d-flex align-items-center gap-1 flex-wrap">
                                <span class="badge bg-success bg-gradient text-white px-2 py-0.5" style="font-size: 0.72rem;">
                                    <i class="bi bi-circle-fill me-1" style="font-size:5px;"></i>Đang mở bán
                                </span>
                                ${goldBadge}
                                ${vosBadge}
                            </div>
                        ...
                        <div class="p-2.5 rounded-3 mb-2" style="background: rgba(3, 20, 16, 0.85); border: 1px solid rgba(255, 209, 102, 0.2); font-size: 0.82rem; line-height: 1.6;">
                            ...
                            <div class="d-flex justify-content-between align-items-center py-0.5">
                                <span class="text-light opacity-75">🏷️ Giá Gốc CĐT:</span>
...
function updateCompareStickyBar() {
    ...
    if (!bar) {
        ...
        bar.style.cssText = `
            ...
            width: 92%;
            max-width: 750px;
            background: linear-gradient(135deg, #07261f 0%, #0c3d32 100%);
    ...
    const unitBadges = selectedCompareUnits.map(code => 
        `<span class="badge bg-warning text-dark fw-bold me-1">${code}</span>`
    ).join('');

    const btnDisabled = selectedCompareUnits.length < 2 ? 'disabled style="opacity:0.6;"' : '';
    const btnText = selectedCompareUnits.length < 2 ? 'Tick thêm 1 căn nữa' : '🚀 XEM SO SÁNH GIÁ 2 CĂN';

    bar.innerHTML = `
        <div class="d-flex align-items-center gap-2 overflow-hidden">
            <span class="fw-bold text-warning small me-1" style="white-space: nowrap;">⚖️ Đã chọn (${selectedCompareUnits.length}/2):</span>
            <div class="d-flex flex-wrap">${unitBadges}</div>
        </div>
        <div class="d-flex align-items-center gap-2">
            <button type="button" class="btn btn-warning fw-bold btn-sm text-dark px-3 py-1.5" ${btnDisabled} onclick="goToSystemCompareTab()" style="border-radius: 20px; white-space: nowrap;">
                ${btnText}
            </button>
            <button type="button" class="btn btn-outline-light btn-sm rounded-circle p-1" onclick="clearSelectedCompareUnits()" title="Bỏ chọn tất cả">
